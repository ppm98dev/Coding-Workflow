...
- **`scripts/migrate-v3-to-v4.sh`** — one-shot deterministic migration for Antigravity-era projects (curl | bash): salvages SPEC → roadmap intent header, keeps phase statuses, **links existing `.quantis/phases/*/…PLAN.md` files into the migrated phase entries** (in-flight phases stay in-flight — `/q-next` resumes their open tasks instead of re-planning), keeps completed phases' `VERIFICATION.md` in place, carries decisions and CONSTITUTION across, archives ALL v3 originals via a catch-all keep-list (incl. milestones, brainstorm/, DEBUG.md — anything a v3 project accumulated) under `.quantis/archive/v3/`, keeps ARCHITECTURE.md/STACK.md in place as planning context, and removes the footprint surgically — only `adapters/ANTIGRAVITY.md` (never the whole dir, which may be app code), root MANIFEST.md and `scripts/validate-*.sh` only when they grep as Quantis's — then installs v4, all in one revertable commit. Smoke-tested end-to-end incl. mid-flight plans and a real-project replica (mixed scripts/, app code in adapters/).
