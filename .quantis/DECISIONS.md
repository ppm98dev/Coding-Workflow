...
### D-013: README/MANIFEST/CHANGELOG are source-only; version marker → `.quantis/VERSION` (Phase 3.5)
**Decision:** `README.md`, `MANIFEST.md`, `CHANGELOG.md` are **Quantis source-only** — `/install`, `/update`, `/upgrade` MUST NEVER write them into a target project (they would clobber the project's own). The Quantis version marker moves from a bare root `VERSION` to **`.quantis/VERSION`** (namespaced; no root collision). MANIFEST classifies the three as source-only; `wf-update` prose explicitly forbids copying them.
**Rationale:** Running `/wf-update` in a real project (ARES_APP) overwrote the project's README/MANIFEST/CHANGELOG. Root cause: `wf-update` Step 5 prose said "replace files listed in the **Core** sections," and MANIFEST listed those three under a "Root Files" (Core) heading — so the agent copied them in, even though the bash only copied `VERSION` (prose and bash disagreed; the agent followed the prose). A bare root `VERSION` also collides with a project's own version file. Fix removes the contradiction (source-only classification + explicit prose guard) and namespaces the version under `.quantis/`.
**Files:** `MANIFEST.md` (source-only restructure), `wf-update/SKILL.md` (prose guard) + 11 version-ref updates (install.sh, upgrade.sh, wf-install, wf-quantis-help, wf-update ×3, wf-upgrade ×2, MANIFEST, README). Tracked in Phase 3.5.
**Relationship:** Complements the dispatch contract (D-011/D-012) — those govern dispatch *behavior*; D-013 governs the install/update *footprint* (what lands in a project).

---

### D-012: Parallel dispatch caps at ≤3 concurrent (waves) + 429 wait-then-inline (Phase 3.4)
**Decision:** Every parallel subagent dispatch is capped at **≤3 concurrent**; more domains go out in **waves of ≤3** (dispatch a wave, await it, dispatch the next). On a `429`/"exhausted capacity" error: do NOT re-dispatch immediately (the "reset after 0s" message is an unreliable upstream passthrough) — wait, retry that one dispatch ONCE, then run the domain inline; re-dispatch failures one at a time (never a simultaneous batch). The rule lives once in `dispatching-parallel-agents/SKILL.md` and is inherited by every fan-out workflow.
**Rationale:** `/wf-stress-test` fired 7 subagents at once → blew the model's per-minute rate limit (`429`), twice, in production. agy has no built-in concurrency cap or backoff — it passes every dispatch to the model, which enforces RPM/ITPM/OTPM ("burst within budget"). The naive "re-dispatch once" retry, misled by "reset after 0s," looped on the limit (thundering herd). Diagnosed via a 3-agent fan-out + web best-practices research (semaphore/wave caps, jittered backoff, respect `Retry-After`). The cap is the load-bearing fix; wait-then-inline is the safety net. **`≤3` is empirically validated** (the user's agy agent recommended it and it runs without crashing) — recorded as a **tunable default, not a hard pin** (drop to 2 if a tier still limits). Deliberately kept simple (no elaborate exponential-backoff-with-jitter spec) because the cap holds.
**Files:** `dispatching-parallel-agents/SKILL.md` (contract); `wf-stress-test` / `wf-research-phase` / `wf-map` / `wf-debug-issue` (waves of ≤3); `antigravity-tools.md` (cap note). Tracked in Phase 3.4.
**Relationship to D-011:** D-011 = every dispatch hands its skill *by path*; D-012 = *how many* dispatch at once and *what to do on a rate limit*. Together they define the dispatch contract.

---

### D-011: The Dispatch Contract — every subagent dispatch hands its methodology skill by path (Phase 3.3)
**Decision:** Every subagent dispatch MUST include `REQUIRED SUB-SKILL: Read and follow .agents/skills/<X>/SKILL.md` for the methodology that subagent embodies — a **path the subagent reads in its own window**, never pasted skill text. A dispatched subagent inherits none of the orchestrator's loaded skills (only a `self` clone would, which we avoid per D-009), so an un-wired dispatch means the subagent simply doesn't have the method.
**Rationale:** agy can dispatch a subagent in any phase. A 4-agent-verified audit (2026-06-13) found the `/wf-*`→methodology chain intact but the dispatched-subagent layer cut off from superpowers — most importantly `test-driven-development` was force-loaded by nothing (SDD only name-dropped "follow TDD"). Handing the **path** (not the text) satisfies both D-009 (lean prompt → no `self`-stall) and D-010 (read what you need in your own context). Canonical correct example already in the codebase: `wf-plan`'s Subagent Planning hands `writing-plans` by path.
**Scope/tracking:** Phase 3.3 (`.quantis/phases/3.3-subagent-superpowers-wiring/`) — 17 tasks; a `validate-dispatch.sh` enforces it going forward.

**D-011a (sub-decision):** `spec-reviewer` subagent → wired to `verification-before-completion`. **Confirmed with user (2026-06-13).** Rationale: a spec review *is* an act of verification — applying the Gate Function (IDENTIFY → RUN → READ → VERIFY) **per spec requirement** is the anti-rubber-stamp discipline that makes a dispatched reviewer worth dispatching, and it makes verification **consistent at both levels**: per-task (SDD spec-reviewer) and per-phase (`/wf-verify`, which already loads this skill).
