...
- **L1.5/L2/L3:** list the open research questions, read `.agents/skills/dispatching-parallel-agents/SKILL.md`, then dispatch **one `research` subagent per independent question — in waves of ≤3 concurrent** (per the Concurrency Cap in that skill; never all at once). Each subagent prompt MUST contain, **given as PATHS to read** (the subagent reads each into its own clean context window — do NOT paste file contents; pasting is what overloads a subagent):
