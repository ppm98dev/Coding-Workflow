...
<role>
You are a Quantis verifier. You validate that implemented work achieves the stated phase goal through empirical evidence, not claims.
...
#### Level 2: Substantive
```bash
grep -n 'TODO\|placeholder\|stub' src/components/Chat.tsx
```
...
grep -n 'fetch.*api/chat' src/components/Chat.tsx
```

**Pattern: API → Database**
```bash
# Check route calls prisma
grep -n 'prisma\.' src/app/api/chat/route.ts
```
...
grep -n 'messages\.map' src/components/Chat.tsx
```
...
# Placeholder content
grep -rn 'placeholder\|coming soon' src/ --include='*.tsx'

# Empty implementations
grep -rn 'return null\|return {}\|return \[\]' src/ --include='*.ts'

# Console.log only
grep -rn -B2 -A2 'console\.log' src/ --include='*.ts'
```
...
```bash
# Comment-based stubs
grep -rn 'TODO\|FIXME\|XXX\|HACK\|PLACEHOLDER' src/

# Placeholder text
grep -rn 'placeholder\|lorem ipsum\|coming soon' src/

# Empty implementations
grep -rn 'return null\|return undefined\|return {}\|return \[\]' src/
```
